...
def get_CIFAR10_partition(opt, partition):
    ...
    if partition in ["train","meanstd"]:
        ...
        cifar = torch.utils.data.Subset(cifar_dataset, subset_indices)
    else:
...
def get_MNIST_partition(opt, partition):
    ...
    if partition in ["train","meanstd"]:
        ...
        mnist = torch.utils.data.Subset(mnist_dataset, subset_indices)
    else:
        raise NotImplementedError
...
def get_accuracy_op(opt, output, target):
    ...
    with torch.no_grad():
        prediction = torch.argmax(output, dim=1)
        return prediction,(prediction == target).sum() 
